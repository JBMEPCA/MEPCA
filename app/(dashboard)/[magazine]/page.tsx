...
export default async function OverviewPage({
  ...
  const [liveCount, upcomingCount, pipelineCount, dueFollowUps, endingSoon, targetCount, nextDeadlines] =
    await Promise.all([
      db.campaign.count({ where: { magazineId: mag.slug, status: "LIVE" } }),
      db.campaign.count({ where: { magazineId: mag.slug, status: "UPCOMING" } }),
      db.pipelineItem.count({
        where: { magazineId: mag.slug, stage: { notIn: ["SIGNED_OFF", "LOST"] } },
      }),
      db.pipelineItem.findMany({
        where: {
          magazineId: mag.slug,
          followUpDate: { lte: now },
          stage: { notIn: ["SIGNED_OFF", "LOST"] },
        },
        orderBy: { followUpDate: "asc" },
        take: 8,
      }),
      db.campaign.findMany({
        where: { magazineId: mag.slug, status: "LIVE", endDate: { gte: now, lte: soon } },
        orderBy: { endDate: "asc" },
        take: 8,
      }),
      db.competitorAdvertiser.count({
        where: { magazineId: mag.slug, goodTarget: true, pitched: false },
      }),
      db.issueDeadline.findMany({
        where: { magazineId: mag.slug, adsDeadline: { gte: now } },
  ...
  const chaseCounts = await Promise.all(
    nextDeadlines.map((d) =>
      db.campaign.count({
        where: { magazineId: mag.slug, issue: shortIssue(d.issue), value: { gt: 0 } },
      })
    )
  );
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">{mag.name}</h1>
        <p className="text-sm text-muted-foreground">Overview</p>
      </div>

      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Live campaigns" value={liveCount} href={`/${mag.slug}/campaigns`} />
        <StatCard label="Upcoming campaigns" value={upcomingCount} href={`/${mag.slug}/campaigns`} />
        <StatCard label="Open pipeline pitches" value={pipelineCount} href={`/${mag.slug}/pipeline`} />
        <StatCard label="Unpitched good targets" value={targetCount} href={`/${mag.slug}/competitor-intel`} />
      </div>
